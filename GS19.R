...
impVCF <- read.vcf("G:\\WAC_Carter_2022_postimp_filt.vcf.gz")

all.pheno = read.csv("G:\\GS\\Chapter3\\allBLUEs.csv") %>%  
...
#
#summary(cnblup)
#save(cnblup,file = paste0(test_year,"_cnblup.Rdata"))
...
#  i.form <- as.formula(paste("cbind(BUAC,",x,") ~ 1"))
#  model <- mmer(fixed = i.form,
#                random=~vsr(line,Gu=G, Gtc=unsm(2)),
...
#save(index,file = paste0(test_year,"_indices.Rdata"))
...
#  i.form <- as.formula(paste("cbind(BUAC,",x,") ~ 1"))
#  model <- mmer(fixed = i.form,
#               random=~vsr(line,Gu=G, Gtc=unsm(3)),
#               rcov=~vsr(units, Gtc=unsm(3)), nIters=30,
#                data=all.pheno, verbose = FALSE)
#  model
  
#}, itt)

#save(multi2,file = paste0(test_year,"_multi2.Rdata"))
...
#  i.form <- as.formula(paste("cbind(BUAC,",x,") ~ 1"))
#  model <- mmer(fixed = i.form,
#                random=~vsr(line,Gu=G, Gtc=unsm(4)),
#                rcov=~vsr(units, Gtc=unsm(4)), nIters=30,
#                data=all.pheno, verbose = FALSE)
#  model
  
#}, itt)

#save(multi3,file = paste0(test_year,"_multi3.Rdata"))


multiblup <- mmer(fixed = cbind(BUAC,NDVI,NWI2,NDRE2,MTVI) ~ 1,
                  random=~vsr(line,Gu=G, Gtc=unsm(5)),
                  rcov=~vsr(units, Gtc=unsm(5)), nIters=30,
                  data=all.pheno, verbose = FALSE)

save(multiblup,file = paste0(test_year,"_multi4.Rdata"))